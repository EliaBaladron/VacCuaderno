...
import com.example.FeedReader.FeedReaderContract_Crotales_SinPoner;
import com.example.FeedReader.FeedReaderDbHelper_VacApp;

import java.util.ArrayList;

public class BD_Crotales_SinPoner extends AppCompatActivity {
    FeedReaderDbHelper_VacApp dbHelper;

    public BD_Crotales_SinPoner(FeedReaderDbHelper_VacApp dbHelperVacApp){
    ...
    public void insertarDatos(){
        insertarDatos(new Crotal("ES 1234 1234 1234", 2));
        insertarDatos(new Crotal("ES 1234 1234 1235", 2));
    }
    ...
    public Crotal insertarDatos(Crotal crotal){
        ...
        values.put(FeedReaderContract_Crotales_SinPoner.FeedEntry.CROTAL, crotal.getCrotal());
        values.put(FeedReaderContract_Crotales_SinPoner.FeedEntry.UNIDADES, crotal.getUnidades());

        // Insert the new row, returning the primary key value of the new row
        //insert() devuelve el ID de la fila recién creada o -1 si hubo un error al insertar los datos.
        long newRowId = db.insert(
                //nombre de la tabla
                FeedReaderContract_Crotales_SinPoner.FeedEntry.TABLE_NAME,
    ...
    public ArrayList<Crotal> getDatosObjetos(){
        ...
        String sortOrder = FeedReaderContract_Crotales_SinPoner.FeedEntry.CROTAL + " ASC";

        Cursor cursor = db.query(
                FeedReaderContract_Crotales_SinPoner.FeedEntry.TABLE_NAME,   // The table to query
    ...
    /*ArrayList<Long> obtenerIDs(Cursor cursor){
        ...
        while(cursor.moveToNext()) {
            long itemId = cursor.getLong(
                    cursor.getColumnIndexOrThrow(FeedReaderContract_Crotales_SinPoner.FeedEntry._ID));
            items.add(itemId);
        }
        cursor.close();
        return items;
    ...
    /*ArrayList<String> obtenerTitulos(Cursor cursor){
        ...
        while(cursor.moveToNext()) {
            String item = cursor.getString(cursor.getColumnIndex(FeedReaderContract_Crotales_SinPoner.FeedEntry.CROTAL));
            items.add(item);
        }
        cursor.close();
        return items;
    ...
    ArrayList<Crotal> getObjetos(Cursor cursor){
        ...
        while(cursor.moveToNext()) {
            long id = cursor.getLong(cursor.getColumnIndex(FeedReaderContract_Crotales_SinPoner.FeedEntry._ID));
            String crotal = cursor.getString(cursor.getColumnIndex(FeedReaderContract_Crotales_SinPoner.FeedEntry.CROTAL));
            int unidades = cursor.getInt(cursor.getColumnIndex(FeedReaderContract_Crotales_SinPoner.FeedEntry.UNIDADES));
    ...
    public void borrarDatos(Crotal crotal){
        ...
        String selection = FeedReaderContract_Crotales_SinPoner.FeedEntry._ID + " LIKE ?";

        String[] selectionArgs = new String[1];
        selectionArgs[0] = crotal.getIdString();

        db.delete(FeedReaderContract_Crotales_SinPoner.FeedEntry.TABLE_NAME, selection, selectionArgs);
